...
- A guard that **throws** — a consumer middleware, or fs-router's own `normalizedRouteToSpecificRoute` on any unmatched URL — is routed by vue-router through `triggerError`, which never reaches `afterEach`. That is a terminal outcome like any other: the window closes, the not-found fallback paints, and one `console.warn` records the failure. Registering that handler also retires vue-router's own `[VUE_ROUTER_R0010]` "register an error handler" warning; the error itself is now reported by fs-router rather than by vue-router's uncaught path.
